...
try:
    ...
    print(f"🔍 DEBUG: ACTIVE_VOICE = {ACTIVE_VOICE}")
    print(f"🔍 DEBUG: VOICES_DATA = {VOICES_DATA}")
    # Get the voices dictionary from VOICES_DATA
    voices_dict = VOICES_DATA.get("voices", {})
    voice_config = voices_dict.get(ACTIVE_VOICE, {})
    print(f"🔍 DEBUG: voice_config for '{ACTIVE_VOICE}' = {voice_config}")
    ELEVENLABS_VOICE = voice_config.get("voice_id", ACTIVE_VOICE)
    ELEVENLABS_MODEL = voice_config.get("model", "eleven_flash_v2_5")
    ...
    ELEVENLABS_VOICE = os.environ.get("ELEVENLABS_VOICE", "qEwI395unGwWV1dn3Y65")
...
class AudioManager:
    ...
    async def _save_tts_to_file(self, text: str, file_path: str):
        ...
        # Filter out backslashes that interfere with ElevenLabs TTS
        filtered_text = text.replace('\\', '')
        
        print(f"🔊 [AudioManager] Generating TTS MP3 for: {filtered_text[:64]}...")
        if text != filtered_text:
            print(f"🔍 [AudioManager] Filtered out backslashes from text")
        print(f"🔍 DEBUG: Using voice ID: {ELEVENLABS_VOICE}")
        print(f"🔍 DEBUG: Using model: {ELEVENLABS_MODEL}")
        
        try:
            audio_data_chunks = []
            for chunk in self.eleven.text_to_speech.stream(
    ...
    def _pygame_playback_worker(self, audio_file: str):
        """Worker function to handle audio playback using pygame"""
        try:
    ...
    async def stop_current_audio(self):
        ...
        # Force join playback thread with short timeout
        if self.playback_thread and self.playback_thread.is_alive():
            self.playback_thread.join(timeout=0.5)
            # Don't wait longer - just mark as stopped
    ...
    async def stop_audio_queue(self):
        ...
        if self.queue_processor_task and not self.queue_processor_task.done():
            ...
            print("Audio queue processor task cancelled.")
        self.queue_processor_task = None
